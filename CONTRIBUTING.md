...
このドキュメントは開発・カスタマイズ向けです。使い方は [README.md](README.md) を参照してください。
...
| `scripts/gen_chat_svg.py` | README の会話例（チャット風SVG）を生成します。セリフはこのファイル内の `CHATS` を編集して再実行してください |
| `docs/illustration-prompts.md` | README 挿絵の画像生成プロンプト（生成画像は `docs/images/` に置きます） |
...
分類・取引区分の一覧は `python3 -m troublelog industries <業種ID>` で確認できます。

**共通化している部分**は、トラブルログのデータモデル、対応時間・時間帯（所定内/時間外/深夜/休日）の集計、取引先CSV、Skill の手順（受付→ドラフト→記録）、共通トーンルール（謝りすぎない・記録を残す・無償の帳尻合わせを約束しない）です。
**業種ごとに差し替える部分**は次のとおりです。
...
1. `troublelog/industries/<新ID>.json` を、既存パックをコピーして作ります。
2. 上表の references / templates / examples を `<新ID>` フォルダで用意します。
3. `python3 -m unittest discover -s tests -t .` を実行します。`tests/test_industry.py` の `test_expected_packs` に新IDを足しておくと、ファイルの欠け・分類名の不一致（JSON と `categories.md`／`focus.md`）・サンプルデータの不正を検出できます。

## セットアップと実行（詳細）

Skill は `.claude/skills/` にあり、このディレクトリで Claude Code を開くと自動で認識されます。明示的に呼ぶ場合は `/trouble-intake` `/trouble-draft` `/trouble-log` です。利用者向けの手順は [README の使い方](README.md#使い方) を参照してください。
...
業種は、登録JSONの `industry` → `--industry` → 環境変数 `TROUBLELOG_INDUSTRY` → `construction` の順で決まります。
業種導入前に作ったDB（`data/troubles.db`）は初回起動時に自動で移行され、既存データは `construction` になります。

### 取引先データのCSVインポート

取り込んだ取引先は、ドラフトの宛名・連絡先の補完と、依頼先候補の提示（分類で絞り込み）に使われます。取引先は業種ごとに管理します。
...
- 文字コードは UTF-8（BOM有無）／Shift_JIS を自動判定します（Excel の「CSV UTF-8」「CSV（コンマ区切り）」どちらでも構いません）。
- 1行目はヘッダーです。同じ業種で**会社名＋担当者**が同じ行は上書きします。1件でも不正な行があれば何も取り込まず、行番号付きでエラーを返します。
- 下表以外の列は無視されます（結果の `ignored_columns` に表示されます）。列構成は全業種共通です。
...
| 担当者 / 役職 | | 同じ会社で担当者が複数なら行を分けます |
| 電話 / メール | | |
| LINE可 | | `可` / `不可`（`○` `×` も可）、空欄は不明 |
| 所在地 / 対応エリア | | |
| 備考 | | 依頼の締切・条件など（依頼文に反映されます） |

| 環境変数 | 既定 | 説明 |
|---|---|---|
| `TROUBLELOG_DB` | `data/troubles.db` | SQLite ファイル |
| `TROUBLELOG_INDUSTRY` | `construction` | 既定の業種 |
| `TROUBLELOG_WORK_START` / `TROUBLELOG_WORK_END` | 業種パックの `work_hours`（全業種 `8` / `17`） | 所定時間（時）。設定すると業種パックより優先されます |

### 時間帯の区分

イベント開始時刻で判定します：`休日`（土日・祝日・振替休日・国民の休日）／`深夜`（平日22〜5時）／`時間外`（平日の所定外）／`所定内`。
`所定内` 以外を「時間外（サビ残相当の候補）」として集計します。
